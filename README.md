...
| AWS Authentication | IRSA | Pods assume IAM roles without any hardcoded keys |
...
| GET | `/readyz` | Readiness probe (checks DynamoDB) |
...
| GET | `/products/{id}/image-url` | Get presigned URL for product image |
...
Every push to `main` triggers a full build and deploy. The image is tagged with the short Git SHA for traceability.
...
EKS pods assume an IAM role via OIDC federation. No AWS keys are stored in the application or Kubernetes secrets. The role is annotated on the Kubernetes ServiceAccount and scoped to only the DynamoDB tables, S3 bucket and Secrets Manager entries it needs.

### DynamoDB Streams + Lambda
Order processing is fully asynchronous. The API writes the order and responds immediately. A DynamoDB Stream triggers Lambda which decrements stock using a conditional update (preventing overselling) and confirms the order. This decouples the API from processing logic.

### Multi-stage Dockerfile
The builder stage installs dependencies and the runtime stage copies only the installed packages. The final image has no build tools, reducing size and attack surface.
...
Helm provides templating, environment-specific values, and atomic rollbacks. The chart includes HPA, topology spread constraints across AZs, liveness and readiness probes, and graceful shutdown hooks.
...
- **Lambda structured logging** — all Lambda logs are JSON-formatted for easy querying
- **CloudWatch Alarms** — alerts on Lambda error rate and p95 duration
- **Log retention** — all log groups have 7-day retention in dev to control costs
...
Bologna, Italy  
[github.com/seed111](https://github.com/seed111) | [linkedin.com/in/abraham-fayemi-0032382a0](https://linkedin.com/in/abraham-fayemi-0032382a0)
